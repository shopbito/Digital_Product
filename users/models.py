...
class User(AbstractBaseUser,PermissionsMixin):
    ...
    username = models.CharField(_('username'), max_length=32, unique=True,
                                help_text=_('required. 30 characters or fewer starting with a letter.letters'),
                                validators=[
                                        validators.RegexValidator(r'^[a-zA-Z][a-zA-Z0-9_\.]+$',
                                                                  _('Enter a valid username starting with a-z.'
                                                                    'This value may contain only letters,number.'
    ...
    phone_number = models.BigIntegerField(_('mobile number'), unique=True, null=True, blank=True,
                                          ...
                                          error_messages={
                                                  'unique': _("A user with this moblie number already exists.")
                                              }
                                          )
    ...
    def get_full_name(self):
        """
        Returns the first_name plus the last_name, with space between.
        """
    ...
    def is_loggedin_user(self):
        ...
        Returns Ture if  user  has actually logged in with valid credentials.
        """
...
class UserProfile(models.Model):
    ...
    avatar = models.ImageField(_('avtar'), blank=True)
    birthday = models.DateField(_('birthday'), blank=True)
    gender = models.BooleanField(_('gender'),help_text=_('female is False,male is True, null is unset'), max_length= 30, blank=True)
